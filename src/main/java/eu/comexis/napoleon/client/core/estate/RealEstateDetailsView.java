...
public class RealEstateDetailsView extends ViewImpl implements
	...
	public void setRealEstate(RealEstate e) {
		...
		String condonium = e.getCondominium();
		condo.setInnerText(condonium);
		
		if (condonium != null && condonium.length() == 0){
			$(emailTenants).hide();
		}else{
			//reset the state of the button
			$(emailTenants).text("Envoyer un email aux locataires").attr("disable", false).show();
		}
		
		$("#sendEmailTenants, #noTenantsEmails").hide();
		...
		email.setInnerHTML(UiHelper.getMailto(e.getAssocEmail(),
				e.getAssocEmail()));
	...
	@UiHandler("emailTenants")
	public void onEmailTenantsClick(ClickEvent e) {
		$(emailTenants).text("Rapatriements des adresses emails...").attr("disable", true);
		presenter.emailTenants();
	}
	...
	public void setEmailsTenants(String emails) {
		...
		if (emails != null && emails.length() > 0){
			$("#sendEmailTenants").attr("href", "mailto:"+emails).fadeIn(100);
		}else{
			$("#noTenantsEmails").fadeIn(100);
		}
