...
public class RealEstateDao extends NapoleonDao<RealEstate> {
	...
	/**
	 * Retrieves the list of all realEstates stored in the database (with the
	...
	public RealEstate update(RealEstate realEstate, Key<Company> companyKey) {
		...
		if (realEstate.getCountry() != null
			...
			City city = countryData.getCityByFullName(country.getId(),
					realEstate.getCity(), realEstate.getPostalCode());
			// if city does not exist, create it.
			if (city == null) {
				city = countryData.addCity(country.getId(),
						realEstate.getCity(), realEstate.getPostalCode());
			}
			if (realEstate.getSquare() != null
					&& !realEstate.getSquare().isEmpty()) {
		...
		if (realEstate.getCondominium() != null
				&& !realEstate.getCondominium().isEmpty()) {
		...
		if (realEstate.getHomeownerAssociation() != null
				&& !realEstate.getHomeownerAssociation().isEmpty()) {
		...
		if (realEstate.getOwner() != null) {
			setOwner(realEstate, realEstate.getOwner().getId(), companyId,
					realEstate.getOwnershipDate());
		}
	...
	public void setOwner(RealEstate realEstate, String ownerId,
			String companyId, Date fromDate) {
		Key<Company> companyKey = new Key<Company>(Company.class, companyId);
		...
		if (currentOwner != null) {
			// if the owner is already the current Owner, we will allow the
			// change the from date if that date is
			...
			if (currentOwner.getOwnerKey().equals(ownerKey)) {
				...
				// if owner is different and the ownership date is more recent
				// than the previous one
				// we stop the ownership of the previous owner and add a new
				// ownership for the current
				if (currentOwner.getFromDate().before(fromDate)) {
					...
					Key<RealEstate> estateKey = new Key<RealEstate>(companyKey,
							RealEstate.class, realEstate.getId());
	...
	public ArrayList<SimpleRealEstate> getListSimpleRealEstatesForOwner(
			String companyId, String ownerId) {
		try {
			Key<Company> companyKey = new Key<Company>(Company.class, companyId);
			Key<Owner> ownerKey = new Key<Owner>(companyKey, Owner.class,
					ownerId);
